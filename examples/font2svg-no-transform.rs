...
// ### TAINT make ID range a command line argument
// ### TAINT make LABEL_FONT_SIZE a command line argument
...
fn process(args: Args) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let glyph_count = if MAX_ID > 0 { MAX_ID as f64 } else { face.number_of_glyphs() as f64 };
    let rows = ( glyph_count / COLUMNS as f64).ceil() as u32;
    ...
    for id in 0..face.number_of_glyphs() {
        ...
        svg.write_attribute("font-size", LABEL_FONT_SIZE);
